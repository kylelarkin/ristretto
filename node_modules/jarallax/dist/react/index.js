...
const jarallax$1 = function jarallax2(items, options, ...args) {
  ...
  for (let index = 0; index < normalizedItems.length; index += 1) {
    ...
    if (typeof options === "object" || typeof options === "undefined") {
      if (!item.jarallax) {
        item.jarallax = new Jarallax$1(item, options);
      }
...
jarallax$1.constructor = Jarallax$1;
...
function createJarallaxRootStyle(style) {
  const nextStyle = {
    ...style || {}
  };
...
function createJarallaxImageStyle(style) {
  const nextStyle = {
    ...style || {}
  };
  if (typeof nextStyle.position === "undefined") {
...
function areOptionsEqual(previousOptions, nextOptions) {
  if (!previousOptions) {
    return false;
  }
  ...
  return nextKeys.every(
    (key) => previousOptions[key] === nextOptions[key]
  );
}
function destroyJarallax(element) {
  if (!element?.jarallax) {
    return;
  }
  jarallax(element, "destroy");
}
function useJarallaxController(config = {}, useVideoExtension = false) {
...
function useJarallax(config = {}) {
  return useJarallaxController(config);
}
function useJarallaxVideo(config = {}) {
  return useJarallaxController(config, true);
}
const Jarallax = forwardRef(function Jarallax2({
  as = "div",
  className,
  children,
  options,
...
const JarallaxVideo = forwardRef(function JarallaxVideo2({
  as = "div",
  className,
  children,
  options,
  ...
  const localRef = useJarallaxVideo({
    options,
    imgElement,
    videoSrc,
    keepImg,
...
const JarallaxImage = forwardRef(
  function JarallaxImage2({ className, style, ...imageProps }, forwardedRef) {
...
export { Jarallax, JarallaxImage, JarallaxVideo, useJarallax, useJarallaxVideo };
